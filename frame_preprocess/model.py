...
def re_normalize(img, mean=IMG_MEAN, std=IMG_STD):
    for b in range(img.shape[0]): # 5
        for t, m, s in zip(img[b], mean, std):
            t.mul_(s).add_(m).sub_(t.min()).div_(t.max())
...
class CRW(nn.Module):
    def __init__(self, args, vis=None):
        ...
        # if args.percept == True:
        #     self.perceptual_loss = get_perceptual_loss("lpips")
        ...
        #         conv(1024, 512, 3, 1, 1),
        #         conv(512, 512, 3, 1, 1),
        #         conv(512, 2, isReLU=False)
        #     )
        #     self.photo_loss = unFlowLoss_unitary()
        #     self.upscale2 = nn.Upsample(scale_factor=2, mode='bilinear')
        #     self.upscale4 = nn.Upsample(scale_factor=4, mode='bilinear')
    ...
    def pixels_to_nodes(self, x):
        ...
        # B N C T h w 10 49 3 4 64 64
        # encoder shape :  torch.Size([490, 3, 4, 64, 64])
        # maps :  torch.Size([490, 512, 4, 8, 8])
    ...
    def forward(self, coc, just_feats=False,):
        ...
        if self.args.is_test == False:
            ...
            #################################################################
            if not self.args.disable_crw:
                if (step % 5000 == 0) and (step >= 1) and  (self.vis is not None): # and False:
                    with torch.no_grad():
