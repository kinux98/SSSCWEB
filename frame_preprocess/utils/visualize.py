...
def nn_patches(vis, P, A_k, prefix='', N=10, K=20):
    ...
    for n,i in enumerate(np.random.permutation(P.shape[0])[:N]):
        ...
        for k in range(I.shape[0]):
            ...
            print("P[I[k, i, :K]] shape : ", P[I[k, i, :K]].shape)
            for ig in range(P[I[k, i, :K]].shape[0]):
                wandb.log({
                    "nn_patches": 
                    wandb.Image(
                        P[I[k, i, :K]][ig], caption='%s_patch_values_%s_%s_%d' % (prefix, n, k,ig)
                    )
                }, commit=False)
...
def frame_pair(x, ff, mm, t1, t2, A, AA, xent_loss, viz):
    ...
    if len(x.shape) < 6:   # Single image input, no patches
        ...
        wandb.log({
            "kpcorr": wandb.Image(pca_ff.transpose(0, -1, 1, 2), caption=f"{t1} {t2}")
        })
        ...
        wandb.log({
            "kpcorr_pca": wandb.Image(torch.stack([pca1,pca2]), caption="pca_viz_combined1")
        })
        # viz.images(torch.stack([img1.cpu(),img2.cpu()]), opts=dict(title=f"{t1} {t2}"), nrow=4, win='pca_viz_combined2')
        wandb.log({
            "kpcorr_img": wandb.Image(torch.stack([img1.cpu(),img2.cpu()]), caption=f"{t1} {t2}")
        })
    ##############################################
    # LOSS VIS
    ##############################################
    ...
    wandb.log({
            "lossvis": wandb.Image(img_grid, caption="lossvis")
        })
