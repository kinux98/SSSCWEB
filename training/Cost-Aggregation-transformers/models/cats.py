...
class TransformerAggregator(nn.Module):
    ...
    def forward(self, corr, source, target, DANN=False):
        B = corr.shape[0]
        x = corr.clone()
        ...
        # print(x.shape) # b x 8 x h x w
        # print(x.mean(1).shape) # b x h x w
        # exit()
        if DANN == False:
            return x.mean(1)
        else:
...
class FeatureExtractionHyperPixel(nn.Module):
    def __init__(self, hyperpixel_ids, feature_size, freeze=False):
        super().__init__()
    ...
    def forward(self, img, custom_size=None):
        ...
        if custom_size is None:
            ...
            for idx, feat in enumerate(feats):
                feats[idx] = F.interpolate(feat, (custom_size, custom_size), None, 'bilinear', True)
...
class CATs(nn.Module):
    ...
    freeze=False):
        ...
        self.proj_feat = nn.ModuleList([
            nn.Conv2d(channels[i], self.feature_proj_dim, 1) for i in hyperpixel_ids
        ])
        self.reduce_cat = nn.Conv2d(128*8, 128, 1)
    ...
    def soft_argmax_corr(self, corr, beta=0.02):
        r'''SFNet: Learning Object-aware Semantic Flow (Lee et al.)'''
        b,_,h,w = corr.size()
    ...
    def mutual_nn_filter(self, correlation_matrix):
        ...
        corr_src = correlation_matrix / corr_src_max.clamp(1e-8)
        corr_trg = correlation_matrix / corr_trg_max.clamp(1e-8)
    ...
    def return_feats_for_DANN(self, target, source):
        src_feats = self.feature_extraction.backbone(source)
        tgt_feats = self.feature_extraction.backbone(target)

        feats = torch.cat([src_feats, tgt_feats], dim=0)
        return feats

    def return_feats_for_DANNv2(self, target, source):
        ...
        for i, (src, tgt) in enumerate(zip(src_feats, tgt_feats)):
            ...
            corrs.append(corr)
            # src : b x c x h x w ->
            # src.flatten(2) : b x c x hw
            # .transpose(-1,-2) : b x hw x c
        ...
        refined_corr = self.decoder(corr, src_feats, tgt_feats, DANN=True)

        return refined_corr
    
    def return_feats(self, target, source):
        src_feats = self.feature_extraction(source)
        tgt_feats = self.feature_extraction(target)
        ...
        for i, (src, tgt) in enumerate(zip(src_feats, tgt_feats)):
            src = F.interpolate(self.proj_feat[i](src), (64, 64), mode='bilinear', align_corners=True)  # b x 128 x h x w
            tgt = F.interpolate(self.proj_feat[i](tgt), (64, 64), mode='bilinear', align_corners=True)  # b x 128 x h x w

            src_feats_proj.append(src)
            tgt_feats_proj.append(tgt)
        
        src_feats = self.reduce_cat(torch.stack(src_feats_proj).transpose(1,0).flatten(1,2))
        tgt_feats = self.reduce_cat(torch.stack(tgt_feats_proj).transpose(1,0).flatten(1,2))# b x 128 x 64 x 64

        return src_feats, tgt_feats

    def forward(self, target, source, get_feat=False, get_corr=False, get_corr_argmax=False, get_feat_DANN=False):
        if (get_feat == True) and (get_corr == False) and (get_feat_DANN == False):
            return self.return_feats(target, source)
        elif (get_feat == False) and (get_corr == False) and (get_feat_DANN == True):
            return self.return_feats_for_DANNv2(target, source)
        elif (get_feat == True) and (get_corr == True) and (get_corr_argmax ==False):
            B, _, H, W = target.size()
            ...
            src_feats_ret = self.reduce_cat(torch.stack(src_feats_proj).transpose(1,0).flatten(1,2))
            tgt_feats_ret = self.reduce_cat(torch.stack(tgt_feats_proj).transpose(1,0).flatten(1,2))# b x 128 x 64 x 64
            ...
            return src_feats_ret, tgt_feats_ret, refined_corr
        else:
        
        # if tgt_pred is not None:
        #     contra_loss = self.semi_contra(target, source, tgt_pred, src_pred)
        #     return contra_loss
...
class Discriminator(nn.Module):
    def __init__(self) -> None:
        ...
        self.conv1 = nn.Conv2d(8, 128, 3, 1, 1, bias=False)
        self.bn1 = norm(128)
    ...
    def forward(self, x, alpha=1.):
        ...
        x = self.avgpool(x).flatten(start_dim=1) # b x c x 1 x 1 -> bc
