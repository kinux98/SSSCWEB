...
from models.feature_backbones import resnet_gelu
...
class FeatureExtractionHyperPixel(nn.Module):
    def __init__(self, hyperpixel_ids, feature_size, freeze=True):
        super().__init__()
        self.backbone = resnet_gelu.resnet101(pretrained=True)
...
class CATs_gelu(nn.Module):
    ...
    def forward(self, target, source, get_feat=False, get_corr=False, get_corr_argmax=False):
        if (get_feat == True) and (get_corr == False):
...
class Discriminator(nn.Module):
    ...
    def forward(self, feat):
        # b x 256 x h x w
        feat = ReverseLayerF.apply(feat, 0.986614)
        feat_pred = self.disc(feat)
...
class Classifier(nn.Module):
    def __init__(self):
        ...
        #     nn.Linear(128, 1, True)
        # )
        self.classifier = resnet.resnet18(True)
        self.classifier.conv1 = nn.Conv2d(128, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        self.classifier.fc = nn.Linear(512, 18, True) # 18 for spair / 20 for pf-pascal

        self.loss = nn.BCEWithLogitsLoss()
    
    def forward(self, feat):
        # feat : b x 128 x h x w
        feat_pred = self.classifier(feat)
