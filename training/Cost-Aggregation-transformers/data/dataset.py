...
class CorrespondenceDataset(Dataset):
    ...
    def __init__(self, benchmark, datapath, thres, device, split, augmentation, feature_size):
        ...
        self.transform_naive = transforms.Compose([transforms.Resize((self.imside, self.imside)),
                                                transforms.ToTensor(),
                                                transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                                    std=[0.229, 0.224, 0.225])])
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'trn' and self.augmentation:
            ...
            batch['src_img_naive'] = self.transform_naive(src_pil)
            batch['trg_img_naive'] = self.transform_naive(trg_pil)
        else:
    ...
    def get_points(self, pts_list, idx, org_imsize):
        ...
        # print(kps) # 2 x n_pts (points + padding (-1))
        # exit(1)
        return kps, n_pts
